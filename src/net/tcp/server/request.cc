...
typedef int state_fn(struct request *req,struct xstring  *dest);
...
int
last_state(struct request *req,struct xstring *dest)
{
	return STATE_DONE;
}

int
state_method(struct request *req,struct xstring  *dest)
{
...
int
state_length(struct request *req,struct xstring  *dest)
{
...
int 
state_body(struct request *req,struct xstring  *dest)
{
    //INFO("in state_body...");
	char c;
	int i=req->pos,c1=0;
...
parse_request(struct request *req)
{
	...
	struct xstring *data=string_new(0);
	struct xstring *dest=data;
	int ret;
	char *tmp;
